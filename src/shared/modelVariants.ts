/**
 * Model family parsing + variant resolution.
 *
 * Ported from the Mousse project: a provider catalog is presented as *families*
 * ("Claude Opus 4.8") with orthogonal Context / Effort / Speed sub-options, and
 * the variant is encoded back into the model id as `model:effort` so the
 * transport stays a plain string.
 */
...
/** Composer label: family plus the selected context/speed sub-options. */
export function getGroupedModelButtonLabel(
  providerId: string,
  modelId: string,
  providers: Array<{ id: string; label: string; models: ModelOption[] }>
): string {
